...
export class AuthService{
    ...
    signup(account: singupModel){
        return this.http.post<AuthResData>('http://127.0.0.1:8000/api/signup/',account)
    ...
    autologin(){
        ...
        const loadedUser = new User(userData.user_id,userData.email,userData.name,userData.token)
    ...
    private handleError(error: HttpErrorResponse){
        ...
        return throwError(() => new Error(errormessage))//throwError( () = > errormessage);
    }

    private handleAuth(res: AuthResData){
        const user = new User(res.user_id,res.email,res.name,res.token);
        this.user.next(user);
